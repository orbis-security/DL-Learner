	...
	public void start() {
		...
		while (!isTerminationCriteriaReached()) {
			...
			if (bestNodeStable.getCovPosMinusCovNeg() < searchTreeStable.best()
				...
				//double testing_acc = ((PosNegLP) learningProblem).testEvaluation(bestNodeStable.getConcept(), false);
				double testing_acc = ((PosNegLP) learningProblem).testEvaluation(searchTreeStable.best().getConcept(), false);

				double actualTrainingTime = getTime() / 1000;
				m_training_times.add(actualTrainingTime);
				m_training_accuracies.add(searchTreeStable.best().getAccuracy());
				m_testing_accuracies.add(testing_acc);

				// no handling needed, it will just look ugly in the output
				logger.info("Time: " + String.valueOf(actualTrainingTime) + "s more accurate (" + acc + ") class expression found: " + renderer.render(searchTreeStable.best().getConcept()));
		...
		((PosNegLP) learningProblem).testEvaluation(bestNodeStable.getConcept(), true);
		helper();
