...
//	private OEHeuristicRuntime heuristicRuntime = new OEHeuristicRuntime();
	...
	private Set<OWLIndividual> testNegativeExamples;
	private Set<OWLIndividual> testPositiveExamples;	
	...
	public void init() throws ComponentInitException {
		...
		if(isClassLearningProblem) {
			...
			testPositiveExamples = ((PosNegLP) learningProblem).getTestPositiveExamples();
			testNegativeExamples = ((PosNegLP) learningProblem).getTestNegativeExamples();
		}
	...
	public void start() {
		...
		//((PosNegLP) learningProblem).testEvaluation(getCurrentlyBestDescription());
		((PosNegLP) learningProblem).testEvaluation(bestEvaluatedDescriptions.getBest().getDescription(), true);
		helper();
...
//			}
	...
	private boolean terminationCriteriaSatisfied() {
		if(maxExecutionTimeInSeconds != 0 && (getTime() >= (maxExecutionTimeInSeconds * 1000)))
		{
			return true;
		}
		...
		(maxExecutionTimeInSecondsAfterImprovement != 0 && ((getTime()) >= (maxExecutionTimeInSecondsAfterImprovement* 1000L))) ||
		(maxExecutionTimeInSeconds != 0 && ((getTime()) >= (maxExecutionTimeInSeconds* 1000L))) ||
	...
	private void showIfBetterSolutionsFound() {
		if(!singleSuggestionMode && bestEvaluatedDescriptions.getBestAccuracy() > currentHighestAccuracy) {
			...
			double testing_acc = ((PosNegLP) learningProblem).testEvaluation(bestEvaluatedDescriptions.getBest().getDescription(), false);

			m_training_times.add(actualTrainingTime);
			m_training_accuracies.add(currentHighestAccuracy);
			m_testing_accuracies.add(testing_acc);

			logger.info("Training time: " + String.valueOf(actualTrainingTime)+ "s more accurate (" + dfPercent.format(currentHighestAccuracy) + ") class expression found after " + durationStr + ": " + descriptionToString(bestEvaluatedDescriptions.getBest().getDescription()));
		}
	}
